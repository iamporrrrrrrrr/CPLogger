...
import { Problem, CFProblem } from '../utils/interfaces.tsx'

function Home({ user }: any) {
    const [loading, setLoading] = useState(false)
    const [problems, setProblems] = useState<Problem[]>([])
    const [newProblemId, setNewProblemId] = useState<string>("")
    const [randomProblem, setRandomProblem] = useState<{ ok: boolean, data: CFProblem }>({
        ok: false,
        data: {
            contestId: 0,
            problemsetName:	'',
    ...
    const updateStatus = (problem: Problem): void => {
    ...
    useEffect(() => {
        ...
        //   })
        setTagStats(tagStats.map((tagStat) => {
    ...
    return (
        <>
                                        ...
                                        {problems.filter((problem => showUnsolved ? !problem.status : true)).map((problem, index) => {
                                            return (
                                                <tr key={`problem-${index}`}>
                                                    ...
                                                    <td className='problem-tags'>{problem.tags.map((tag: string, index2) => {
                                                        return (
                    ...
                    <div className="chart">
                        ...
                        <div className="chart-container">
                            <PieChart
                                series={[
                                    {
                                        data: tagStats.sort((a, b) => b.value - a.value),
                                        innerRadius: 30,
                                        outerRadius: 100,
                                        cx: '82%',
                                        cy: '50%'
            ...
            {randomProblem.ok && randomProblem.data.name?.length &&
